...
document.querySelectorAll('[data-household-tab]').forEach(button=>button.addEventListener('click',()=>showHouseholdTab(button.dataset.householdTab)));
document.querySelectorAll('[data-household-tab-target]').forEach(button=>button.addEventListener('click',()=>showHouseholdTab(button.dataset.householdTabTarget)));
document.querySelectorAll('.candidate-save').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();const active=button.classList.toggle('saved');button.textContent=active?'♥':'♡';const status=button.closest('.candidate-card').querySelector('.candidate-status');status.textContent=active?'FAVORITO':'PENDIENTE';status.className=`candidate-status${active?' favorite':''}`;ping(active?'Marcado como favorito del Hogar':'Devuelto a pendientes')}));
document.querySelectorAll('.candidate-card').forEach(card=>card.addEventListener('click',()=>{const id=card.dataset.candidate==='ponzano'?1:card.dataset.candidate==='arguelles'?5:2;showDetail(listings.find(item=>item.id===id))}));
document.querySelector('#inviteHousehold').addEventListener('click',()=>{showHouseholdTab('invitations');ping('Comparte el enlace para invitar a alguien')});
['copyHouseholdLink','copyInviteLink'].forEach(id=>document.querySelector(`#${id}`).addEventListener('click',()=>{navigator.clipboard?.writeText('https://rooms.app/hogar/ponzano-24').catch(()=>{});showHouseholdTab('invitations');ping('Enlace de invitación copiado')}));
document.querySelector('#householdChatForm').addEventListener('submit',event=>{event.preventDefault();const input=document.querySelector('#householdChatInput'),message=input.value.trim();if(!message)return;const bubble=document.createElement('div');bubble.className='chat-message mine';bubble.innerHTML=`<p>${message.replace(/[<>]/g,'')}</p><small>Ahora</small>`;document.querySelector('.group-chat').appendChild(bubble);input.value='';bubble.scrollIntoView({behavior:'smooth',block:'center'})});
document.querySelector('#communityBack').addEventListener('click',()=>showMainView('home'));
document.querySelectorAll('[data-community-tab]').forEach(button=>button.addEventListener('click',()=>{const name=button.dataset.communityTab;document.querySelectorAll('[data-community-tab]').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('[data-community-panel]').forEach(panel=>panel.hidden=panel.dataset.communityPanel!==name);window.scrollTo({top:document.querySelector('.community-tabs').offsetTop-70,behavior:'smooth'})}));
document.querySelectorAll('[data-community-vote]').forEach(button=>button.addEventListener('click',()=>{const active=button.classList.toggle('active');button.textContent=active?'✓ Votado':'↑ Votar';const counter=button.closest('.community-post').querySelector('[data-community-votes]');if(counter)counter.textContent=Number(counter.textContent)+(active?1:-1)}));
document.querySelectorAll('[data-community-save]').forEach(button=>button.addEventListener('click',()=>{const active=button.classList.toggle('active');button.textContent=active?'♥ Guardado':'♡ Guardar';ping(active?'Publicación guardada':'Eliminada de guardados')}));
document.querySelectorAll('[data-admin-menu]').forEach(button=>button.addEventListener('click',()=>open(adminModerationModal)));
document.querySelector('.community-main').addEventListener('click',event=>{const listing=event.target.closest('[data-listing-id]');if(listing)showDetail(listings.find(item=>item.id===Number(listing.dataset.listingId)))});
...
document.querySelector('#markNotificationsRead').addEventListener('click',()=>{document.querySelectorAll('[data-notification]').forEach(item=>item.classList.add('read'));document.querySelector('#openNotifications i').hidden=true;ping('Todo marcado como leído')});
document.querySelectorAll('[data-notification]').forEach(item=>item.addEventListener('click',()=>{item.classList.add('read');const copy=item.textContent;if(copy.includes('Nueva habitación')){closeAll();showDetail(listings[0])}else if(copy.includes('Lucía quiere')){closeAll();openConversation('lucia')}else if(copy.includes('ha añadido un piso')||copy.includes('Todo el Hogar')){closeAll();showMainView('household')}else if(copy.includes('publicación destacada')){closeAll();openCommunity()}else if(copy.includes('Nuevo interesado')){closeAll();openConversation('listing')}else if(copy.includes('Sigue disponible'))ping('Anuncio confirmado como disponible')}));
...
function updateCompareBar(){const count=document.querySelectorAll('[data-compare-item]:checked').length;document.querySelector('#compareCount').textContent=count;document.querySelector('#compareBar').hidden=count===0;document.querySelector('#compareHomes').disabled=count<2}
document.querySelectorAll('[data-compare-item]').forEach(input=>input.addEventListener('change',updateCompareBar));
document.querySelector('#compareHomes').addEventListener('click',()=>{if(document.querySelectorAll('[data-compare-item]:checked').length<2){ping('Selecciona dos viviendas');return}open(compareModal)});
document.querySelectorAll('#savedView [data-open-community]').forEach(button=>button.addEventListener('click',openCommunity));
...
feed.addEventListener('click',event=>{const property=event.target.closest('[data-listing-id]'),userCard=event.target.closest('[data-user-card]'),communityCard=event.target.closest('[data-open-community]'),carouselControl=event.target.closest('[data-carousel-prev],[data-carousel-next]'),action=event.target.closest('[data-action]');if(carouselControl){event.stopPropagation();advanceCarousel(carouselControl.closest('[data-carousel]'),carouselControl.hasAttribute('data-carousel-next')?1:-1);return}if(action){event.stopPropagation();if(action.dataset.action==='discard'){const card=action.closest('.feed-card');card.classList.add('discarding');setTimeout(()=>{card.hidden=true;ping('Lo tendremos en cuenta para tus próximos matches')},220);return}action.classList.toggle('saved');action.innerHTML=action.classList.contains('saved')?'♥ <span>Guardado</span>':'♡ <span>Guardar</span>';ping(action.classList.contains('saved')?'Guardado en tus favoritos':'Eliminado de favoritos');return}const imageSave=event.target.closest('.feed-save');if(imageSave){event.stopPropagation();imageSave.classList.toggle('saved');imageSave.textContent=imageSave.classList.contains('saved')?'♥':'♡';ping(imageSave.classList.contains('saved')?'Guardado en tus favoritos':'Eliminado de favoritos');return}const vote=event.target.closest('[data-vote]');if(vote){const count=document.querySelector('[data-vote-count]'),active=vote.classList.toggle('voted');count.textContent=Number(count.textContent)+(active?1:-1);return}if(event.target.closest('[data-toast]'))return;if(property)showDetail(listings.find(item=>item.id===Number(property.dataset.listingId)));else if(userCard)open(userProfileModal);else if(communityCard)openCommunity()});
feed.addEventListener('keydown',event=>{if(event.key!=='Enter')return;if(event.target.matches('[data-listing-id]'))showDetail(listings.find(item=>item.id===Number(event.target.dataset.listingId)));if(event.target.matches('[data-user-card]'))open(userProfileModal);if(event.target.matches('[data-open-community]'))openCommunity()});

document.querySelector('#exploreFilters').addEventListener('click',()=>open(exploreFilterModal));
document.querySelectorAll('[data-explore-view]').forEach(button=>button.addEventListener('click',()=>{const map=button.dataset.exploreView==='map';document.querySelectorAll('[data-explore-view]').forEach(item=>item.classList.toggle('active',item===button));document.querySelector('#exploreList').hidden=map;document.querySelector('#exploreMap').hidden=!map}));
document.querySelector('#exploreSort').addEventListener('change',event=>ping(`Ordenado por ${event.target.options[event.target.selectedIndex].textContent.toLowerCase()}`));
document.querySelector('#exploreList').addEventListener('click',event=>{const save=event.target.closest('[data-explore-save]'),card=event.target.closest('[data-listing-id]');if(save){save.classList.toggle('saved');save.textContent=save.classList.contains('saved')?'♥':'♡';ping(save.classList.contains('saved')?'Guardado en favoritos':'Eliminado de favoritos');return}if(card)showDetail(listings.find(item=>item.id===Number(card.dataset.listingId)))});
document.querySelectorAll('[data-map-listing]').forEach(pin=>pin.addEventListener('click',()=>{document.querySelectorAll('[data-map-listing]').forEach(item=>item.classList.toggle('active',item===pin));const item=listings.find(listing=>listing.id===Number(pin.dataset.mapListing));document.querySelector('#mapMiniCard').innerHTML=`<img src="${item.img}" alt="${item.title}"><div><span>${item.match}% MATCH</span><h2>${item.zone} · ${item.id===3?'Piso entero':'Habitación'}</h2><p><b>${item.price} €</b> / mes</p></div>`}));
document.querySelector('#mapMiniCard').addEventListener('click',()=>{const pin=document.querySelector('[data-map-listing].active');showDetail(listings.find(item=>item.id===Number(pin.dataset.mapListing)))});
...
document.querySelector('#confirmExternalAvailable').addEventListener('click',event=>{event.currentTarget.textContent='Disponibilidad confirmada ✓';event.currentTarget.disabled=true;document.querySelectorAll('.availability-warning').forEach(item=>item.textContent='CONFIRMADO POR LA COMUNIDAD');ping('Gracias · disponibilidad actualizada')});
document.querySelectorAll('[data-empty-action]').forEach(button=>button.addEventListener('click',()=>{if(button.dataset.emptyAction==='similar'){document.querySelector('#exploreSearch').value='';document.querySelector('#exploreEmpty').hidden=true;document.querySelector('#exploreList').hidden=false;document.querySelectorAll('.compact-property,#exploreList .explore-exception,#exploreList .explore-count').forEach(item=>item.hidden=false);ping('Mostrando opciones similares');return}open(exploreFilterModal)}));
